  ...
  <nav class="navbar">
    ...
    <?php
		$bno = $_GET['idx']; /* bno함수에 idx값을 받아와 넣음*/
                        $b = mysqli_real_escape_string($connection, $bno);	
		$sql = "select * from board where number='$b';"; /* 받아온 idx값을 선택 */
		$sql = mysqli_query($connection, $sql);
		$query_data = mysqli_fetch_row($sql);
	?>
<!-- 글 불러오기 -->
<div id="board_read" style="margin: 0 auto">
	<h2>제목 : <?php echo $query_data[3]?></h2>
		<div id="user_info">
			작성자 : <?php echo $query_data[1]?> 일자 : <?php echo $query_data[5]?>
				<div id="bo_line"></div>
			</div>
			<div id="bo_content">
				<?php echo $query_data[4]?>
			</div>
	<!-- 목록, 수정, 삭제 -->
	<div id="bo_ser">
		<ul>
			<li><a href="./commu.php">[목록으로]</a></li>
		</ul>
	</div>
</div>
    </section>
